...
const VendorDetailsForPurchaseDrawer = ({
   vendorData,
   enquiryId,
   bid_documents = [],
}) => {
      ...
      <Wrapper>
         <Flex sx={{ mb: 1, alignItems: 'center' }}>
            <Typography variant='h5'>
               {vendorData?.others?.bid_details?.vendor_name}
            </Typography>
            <a
               href={`https://solruf.com/portfolio/${vendorData?.others?.vendor?.portfolio?.slug}`}
               alt='go to portfolio'
               target='_blank'
               rel='noreferrer'
            >
         ...
         >
            <Box sx={{ mr: 2, minWidth: '50%' }}>
               <FeatureDetail
                  title='Bidding Price'
                  value={
                     vendorData?.others?.total_price /
                     vendorData?.others?.quantity
                  }
                  icon={<LocalAtmIcon />}
               />
               <FeatureDetail
                  title='Advanced Payment'
                  value={vendorData?.others?.details[0]?.advance_payment}
                  icon={<PaymentsIcon />}
               />
               ...
               <FeatureDetail
                  ...
                  title='Total Price'
                  value={vendorData?.others?.total_price}
            ...
            <Box>
               <FeatureDetail
                  title='Booking Price'
                  value={vendorData?.others?.booking_price}
                  icon={<LocalAtmIcon />}
               />

               <FeatureDetail
                  title='Warranty Years'
                  value={vendorData?.others?.warranty_years}
               ...
               <FeatureDetail
                  ...
                  title='Description'
                  value={vendorData?.others?.bid_details?.description}
                  icon={<DescriptionIcon />}
               />
            </Box>
         </Flex>
...
export default VendorDetailsForPurchaseDrawer;
